...
echo "adding distrobox/boxbuddy, flathub and preferred flatpaks" && sleep 1
installThings() {

	echo "generic curl cmd for distrobox" && sleep 1
	curl -s https://raw.githubusercontent.com/89luca89/distrobox/main/install | sudo sh
	echo "boxbuddy: distrobox gui" && sleep 1
	flatpak install flathub io.github.dvlv.boxbuddyrs

	echo "adding unfiltered flathub" && sleep 1
	flatpak remote-add --if-not-exists flathub https://flathub.org/repo/flathub.flatpakrepo

	echo "installing preferred flatpaks" && sleep 1
